...
type StopEditorControlsProps = {
    ...
    onUpdateStopName: (index: number, name_th: string, name_en: string, icon?: string) => void;
};
...
export function StopEditorControls({
    ...
    return (
        <div className="glass-card p-4 animate-slideUp min-w-[240px]">
...
export function useStopEditor() {
    ...
    const updateStopName = useCallback(
        (index: number, name_th: string, name_en: string, icon?: string) => {
            setStops((prev) => {
                const newStops = [...prev];
                newStops[index] = { ...newStops[index], name_th, name_en, icon };
                return newStops;
            });
